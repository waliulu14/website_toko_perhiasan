<?php
// edit_produk.php (halaman untuk mengedit produk)
...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        // Redirect ke halaman data produk jika produk tidak ditemukan
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Upload gambar jika ada file gambar baru yang diunggah
    if (!empty($_FILES['gambar_produk']['name'])) {
        ...
        if (empty($errors)) {
            if (move_uploaded_file($_FILES["gambar_produk"]["tmp_name"], $target_file)) {
                // Query update produk ke dalam tabel
                $query = "UPDATE produk SET id_kategori = '$id_kategori', nama = '$nama_produk', harga = '$harga_produk', gambar = '$target_file' WHERE id = '$id_produk'";
                $result = mysqli_query($conn, $query);

                if ($result) {
                    // Redirect ke halaman data produk setelah berhasil mengupdate produk
                    header('Location: data_produk.php');
                    exit();
                } else {
                    $errors[] = 'Terjadi kesalahan. Gagal mengupdate produk.';
                }
            } else {
        ...
        $query = "UPDATE produk SET id_kategori = '$id_kategori', nama = '$nama_produk', harga = '$harga_produk' WHERE id = '$id_produk'";
        $result = mysqli_query($conn, $query);
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="id_kategori" name="id_kategori">
                                <?php while ($row_kategori = mysqli_fetch_assoc($result_kategori)): ?>
                                    <option value="<?php echo $row_kategori['id']; ?>" <?php if ($row_kategori['id'] == $id_kategori)
                                           echo 'selected'; ?>><?php echo $row_kategori['nama_kategori']; ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="formcontrol" id="gambar_produk" name="gambar_produk">
                        </div>
